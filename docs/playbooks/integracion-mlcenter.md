...
`LinkResultado` obligatorio en `FINALIZADO`. Errores: `{ codigo, mensaje, detalles[] }`
(400 validación / transición inválida; 401 sin token; 500). Doc fuente:
`API_ENDPOINT_ACTUALIZAR_ESTADO_AUTOCHECK.pdf`.
...
Estado: **scaffold listo y verificado** (smoke: 401 sin secret · 200 `skipped` con stage no
mapeado · 400 payload inválido). El adapter HTTP real está implementado pero **sin probar**
contra el sandbox de ML (faltan credenciales).

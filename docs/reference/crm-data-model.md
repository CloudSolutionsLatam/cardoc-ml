...
> (lookup → `Inspectores`, ya listado en "Agenda"; `Inspector.name` = nombre del técnico, verificado
> en vivo); `empresa` = constante **`"Certia"`**. El re-notify de `PENDIENTE` (`Nueva Solicitud`)
> queda en observación por el anti-duplicados de v1.1.

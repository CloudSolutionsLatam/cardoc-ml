...
| **OQ-N7** | Origen del `LinkResultado` que se envía a ML en `FINALIZADO`. **Resuelto (Nestor 2026-07-15):** es el endpoint **D3b autenticado de cardoc** `/v1/informes/solicitud/{nroSolicitud}/pdf` (lo arma el Deluge). Requiere Bearer + scope `reports:pdf`; **ML/los consumidores saben cómo consumirlo** (no necesita ser público). Validado en vivo: `FINALIZADO` con ese `linkResultado` → `sent` real contra testing. | E-07 | ✅ resuelta |
| **OQ-N10** | **Fuente de `NombreTecnico`/`Empresa` (contrato ML v1.1, obligatorios en toda actualización).** **Decidido (Nestor 2026-07-15): los manda el CRM en el webhook** — implementado end-to-end (`dealEstadoSchema` + use-case exige ambos para stages notificables → `422` si faltan). `nombreTecnico` = `Deals.Inspector` (lookup → `Inspectores`; **verificado en vivo 2026-07-15**: `Inspector.name` = nombre del técnico, ej. "Sebastian Quintana"). `empresa` = **constante `"Certia"`** (hardcodeada en el Deluge, Nestor 2026-07-15). **`PENDIENTE` — decidido (Nestor 2026-07-15):** se mantiene `Nueva Solicitud → PENDIENTE`; como en el alta no hay inspector (y ML aplica anti-duplicados), esa notificación termina en `422` — **aceptado como no-op inocuo ("no afecta")**: no reintenta, no llama a ML de más, solo queda en `audit_log`. NO cambiar a `skipped` sin OK. | E-07 · v1.1 · [integracion-mlcenter](playbooks/integracion-mlcenter.md) | ✅ resuelta (campos + PENDIENTE decididos) |

...
| **OQ-P10** | **Acceso server-to-server a Creator** para el detalle del informe (E-03). cardoc-ml NO puede usar el SDK cliente `ZOHO.CREATOR.DATA.invokeCustomApi` del portal. Falta confirmar: (a) **URL REST exacta** de `GET_INSPECTION_REPORT_DETAIL` invocable server-to-server; (b) **auth** — ¿`public_key` sola?, ¿qué reemplaza al `token` de sesión de portal?, ¿OAuth self-client con scopes de Creator?; (c) **descarga de WorkDrive** para las fotos (URL directa vs API `/files/{id}/content`) y su auth. Aislado en `creator-client.ts` (config-driven: `CREATOR_REPORT_DETAIL_URL`, `CREATOR_PUBLIC_KEY`). Sin esto no se activa `CARDOC_REPORTS_MODE=creator`. | E-03 (adapter Creator) · [ADR-0012](decisions/README.md#adr-0012) | 🔴 abierta |

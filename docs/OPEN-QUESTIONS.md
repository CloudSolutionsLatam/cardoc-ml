...
| **OQ-N6** | Mapeo estado CRM → ML `Estado` (`COORDINACIÓN` / `FINALIZADO`). **Hallazgos (2026-06-30):** (a) el field-tracker `Historial_de_Estado` trackea `Informes_Revision.Estado`, **no** `Deals.Stage` → decidir la fuente; (b) pipeline = **B2B** (Nueva Solicitud → Agendado B2B → Completado → Cerrado/Cancelado). **Candidato a confirmar:** `Agendado B2B`→`COORDINACIÓN`, `Completado`/`Cerrado`→`FINALIZADO`. `STAGE_TO_ESTADO` placeholder. Ver [`reference/crm-data-model.md`](reference/crm-data-model.md). | E-07 · [ADR-0013](decisions/README.md#adr-0013) | 🟡 candidato |

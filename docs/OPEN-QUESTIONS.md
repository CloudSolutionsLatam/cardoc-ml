...
| **OQ-N8** | **Tamaño del PDF por volumen de fotos.** Smoke e2e (id `4837888000004307360`): **132 componentes, 302 fotos**. ✅ **Velocidad resuelta:** prefetch de fotos en paralelo (tope de concurrencia) → **~93s → ~17s**. **Residual 🔴 (tamaño):** el PDF pesa **126 MB** (pdf-lib embebe los bytes crudos; achicar el `drawImage` no reduce el peso) — inviable para el cliente y para el tope de Advanced I/O. **Decidir:** (a) downscale/re-encode de cada foto (lib JS pura tipo jimp, sin binario); (b) thumbnails de WorkDrive; (c) ajustar el cap (hoy `MAX_PHOTOS=6`/componente) + resolución objetivo. Y para requests repetidas, el **write-back con caché** (ADR-0012). | E-03 (PDF producción) · [ADR-0012](decisions/README.md#adr-0012) | 🟡 velocidad OK; tamaño pendiente |
